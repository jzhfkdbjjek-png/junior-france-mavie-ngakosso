  ...
  const content = (
    <>
      {loading ? (
        ...
        <span className="cinematic-btn-inner flex items-center justify-center gap-2 whitespace-nowrap">
          {icon && iconPosition === 'left' && (
            <span className="cinematic-btn-icon-wrapper shrink-0 transition-transform duration-300 group-hover:scale-110">
              {icon}
            </span>
          )}
          {children && <span className="cinematic-btn-text tracking-wider">{children}</span>}
